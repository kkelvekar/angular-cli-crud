...
export class ProductService {
    private productUrl = 'http://kkcrudapi.azurewebsites.net/Product';
    ...
    addProduct(product: IProduct): Observable<any> {
        const productRequest = JSON.stringify(this.mapProductToResponse(product));
        return this.http.post(`${this.productUrl}/PostProduct`, productRequest, this.httpHelper.jsonRequestOptions);
    }

    updateProduct(product: IProduct): Observable<any> {
        const productRequest = this.mapProductToResponse(product);
